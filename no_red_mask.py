...
input_raster = r"D:\Luba\chapter3\python_code_workflow\input_images\image_little.tif"
...
extracted_raster_both_bands.save("extracted_raster_only_red.tif")

# Step 7: Convert to a vector layer and filter by size
dead_trees_region = arcpy.sa.RegionGroup('extracted_raster_only_red.tif')

arcpy.conversion.RasterToPolygon(dead_trees_region, "dead_trees_vector_no_red.shp", "NO_SIMPLIFY")

# Add a new field named "Shape_Area" to store the area
arcpy.AddField_management("dead_trees_vector_no_red.shp", "Shape_Area", "DOUBLE")

# Calculate the area and populate the "Shape_Area" field
expression = "!shape.area!"
arcpy.CalculateField_management("dead_trees_vector_no_red.shp", "Shape_Area", expression, "PYTHON")

in_feature = "dead_trees_vector_no_red.shp"
out_feature = "dead_trees_selected_no_red.shp"
where_clause = "Shape_Area > 2"
arcpy.analysis.Select("dead_trees_vector_no_red.shp", out_feature, where_clause)

arcpy.management.CopyFeatures("dead_trees_selected_no_red.shp", 'dead_trees_selected_copy_no_red.shp')


# Step 8: Buffer dead trees, dissolve and filter by size:
in_features = 'dead_trees_selected_copy_no_red.shp'
out_feature_class = 'buffered_trees_no_red.shp'
...
in_buffer = 'buffered_trees_no_red.shp'
dissolved = 'dissolved_buffer_no_red.shp'
arcpy.management.Dissolve(in_buffer, dissolved, "", "", "SINGLE_PART")

# Add a new field named "Shape_Area" to store the area
arcpy.AddField_management("dissolved_buffer_no_red.shp", "Shape_Area", "DOUBLE")

# Calculate the area and populate the "Shape_Area" field
expression = "!shape.area!"
arcpy.CalculateField_management("dissolved_buffer_no_red.shp", "Shape_Area", expression, "PYTHON")

out_feature = "trees_buffer_processed_no_red.shp"
where_clause = '"Shape_Area">80'
arcpy.analysis.Select("dissolved_buffer_no_red.shp", out_feature, where_clause)

in_rast = "trees_buffer_processed_no_red.shp"
out_rast = "dead_trees_final_no_red.shp"
...
target_feature = "dead_trees_final_no_red.shp"
join_feature = "Molly_points_clipped.shp"
out_feature_class = "polygons_to_points_no_red.shp"
arcpy.analysis.SpatialJoin(target_feature, join_feature, out_feature_class)

target_feature = "Molly_points_clipped.shp"
join_feature = "dead_trees_final_no_red.shp"
out_feature_class = "points_to_polygons_no_red.shp"
arcpy.analysis.SpatialJoin(target_feature, join_feature, out_feature_class)

#Calculate number rows which had intersection with ground data

# Specify the input feature classes
polygons_to_points_no_red = "polygons_to_points_no_red.shp"
points_to_polygons_no_red = "points_to_polygons_no_red.shp"


# Use the Select tool to select rows where Join_Count is not equal to 0 for polygons_to_points
arcpy.Select_analysis(polygons_to_points_no_red, "selected_polygons_to_points_no_red", '"Join_Count" <> 0')

# Use the Get Count tool to count selected features and total features for polygons_to_points
TP = int(arcpy.GetCount_management("selected_polygons_to_points_no_red").getOutput(0))
...
arcpy.Select_analysis(points_to_polygons_no_red, "selected_points_to_polygons_no_red", '"Join_Count" <> 0')

# Use the Get Count tool to count selected features and total features for points_to_polygons
TP_2 = int(arcpy.GetCount_management("selected_points_to_polygons_no_red").getOutput(0))
All_points = int(arcpy.GetCount_management(points_to_polygons_no_red).getOutput(0))
FP = All_points - TP_2
